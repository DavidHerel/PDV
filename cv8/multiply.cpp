...
sparse_vector merge(const sparse_vector & a, const sparse_vector & b) {
    ...
    try {
        ...
        while (index_a < a_size && index_b < b_size){
            //if a < b, increase a and store a to result
            if ( a_entries[index_a].index < b_entries[index_b].index ){
                result.set(a_entries[index_a].index, a_entries[index_a].value);
                index_a++;

            }
            // if opposite, increase b and store b to result
            else{
                result.set(b_entries[index_b].index, b_entries[index_b].value);
                index_b++;
            }
        }
    //if we ended up on a we will not add it on the end
        //in other case we will add it
        while (index_a < a_size){
            result.set(a_entries[index_a].index, a_entries[index_a].value);
            index_a++;
        }
        //if we ended up on a we will not add it on the end
