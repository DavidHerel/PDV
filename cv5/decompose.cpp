...
#pragma omp parallel for schedule(guided)
    for (unsigned int j = 0; j < size; j++){
        ...
        unsigned long temp_min = min;
        while (steps < temp_min) {
            if (min.compare_exchange_strong(temp_min,steps)) break;
        }
    }
...
unsigned long findn_parallel(unsigned long criteria) {
    ...
    std::atomic<unsigned long> final_i {0};
...
unsigned long long fibonacci_parallel_worker(unsigned int n) {
    if(n <= 2) {
        ...
        x = fibonacci_parallel_worker(n - 1);
        #pragma omp task shared(y)
        y = fibonacci_parallel_worker(n - 2);
        #pragma omp taskwait
        return x + y;
    }
}